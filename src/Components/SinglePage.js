...
const SinglePage = (props) => {
  ...
  const fetchVideo = async (trailer) => {

    const response = await fetch(
      `https://www.googleapis.com/youtube/v3/videos?part=snippet&id=${trailer}&key=${process.env.REACT_APP_API_KEY}`
    );
  ...
  useEffect(() => {
    ...
    const fetchMovieData = async () => {
      try {
        const moviefound = allmovies.find((movie) => movie.movie_id === params.movie_id)
        if (moviefound) {
          setData(moviefound);
          fetchVideo(moviefound.movie_trailer);
        }
      } catch (error) {
    ...
    <>    {
      Data ?
        < Container >
          ...
          <Recommandations movies={allmovies} />
